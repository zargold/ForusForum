...
db.run("INSERT INTO posts (title, body, imageUrl, timeLive, timeSticky, userID, catID, tagA, tagB, tagC, vote, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "What is Six Times Seven?", "Have you noticed that 6 * 7 is 42 woahhh that's like totally a conspiracy maannn. It's like if you search through the dictionary and take every 5th letter of every 393 word subtracting 3 and adding 5 at increments of every 4392 words you'll totally spell dude.", "http://s3.amazonaws.com/kym-assets/photos/images/original/000/141/327/sun.jpg?1309269207", "60", "8888", "1", "1", "42", "bizarre", "conspiracy", "1", "CURRENT_TIMESTAMP", function(err) {
  if (err) console.log(err);
});

// comments ( id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, body TEXT, userID TEXT, postID INTEGER, updated_at REAL);
db.run("INSERT INTO comments (title, body, userID, postID, created_at) VALUES (?, ?, ?, ?, ?)", "Your momma is 42!", "oooohhhhh, and I'm not talking about her age!", "1", "1", "CURRENT_TIMESTAMP", function(err) {
  if (err) console.log(err);
});
// firstN TEXT, lastN TEXT, email TEXT, imageUrl TEXT, image BLOB, password TEXT, vote INTEGER, avatar TEXT,
db.run("INSERT INTO users (firstN, lastN, userN, email, imageUrl, image, password, vote, avatar, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", "George", "Smith", "Gunsmith", "cyberninjajuice@gmail.com", "http://s3.amazonaws.com/kym-assets/photos/images/original/000/141/327/sun.jpg?1309269207", "", "passWD", "1", "N", "CURRENT_TIMESTAMP", function(err) {
  if (err) console.log(err);
});